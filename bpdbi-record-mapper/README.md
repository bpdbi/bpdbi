...
A reflection-based `RowMapper` for [bpdbi](../) that automatically maps query result rows to Java
records
...
With `bpdbi-kotlin`, use `@Serializable` data classes instead of reflection:
...
With `bpdbi-kotlin`, use `@SerialName` from kotlinx.serialization:
...
`bpdbi-kotlin` supports all the above types plus:
...
With `bpdbi-kotlin`, null handling is type-safe at compile time:
