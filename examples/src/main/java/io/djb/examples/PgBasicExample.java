...
/**
 * Basic PostgreSQL usage: connect, create a table, insert, query.
 *
 * Run: ./gradlew :examples:run -PmainClass=io.djb.examples.PgBasicExample
 * Requires: PostgreSQL running on localhost:5432 with database "postgres"
 */
public class PgBasicExample {

    public static void main(String[] args) {
        try (var conn = PgConnection.connect("localhost", 5432, "postgres", "postgres", "postgres")) {
            System.out.println("Connected to PostgreSQL " + conn.parameters().get("server_version"));
            ...
            conn.query("INSERT INTO users (name, email) VALUES ($1, $2)", "Charlie", "charlie@example.com");
            ...
            // Update
            var updated = conn.query("UPDATE users SET email = $1 WHERE name = $2", "bob@newdomain.com", "Bob");
