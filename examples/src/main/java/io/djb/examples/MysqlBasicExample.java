 ...
 *
 * Run: ./gradlew :examples:run -PmainClass=io.djb.examples.MysqlBasicExample
 * Requires: MySQL running on localhost:3306 with database "test"
 */
public class MysqlBasicExample {
    ...
    public static void main(String[] args) {
        try (var conn = MysqlConnection.connect("localhost", 3306, "test", "root", "password")) {
            ...
            // Create a table
            conn.query("CREATE TEMPORARY TABLE products (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), price DECIMAL(10,2))");
            ...
            for (var row : rs) {
                System.out.printf("  id=%d name=%-10s price=$%s%n",
