...
public class PgPipeliningExample {
    ...
    public static void main(String[] args) {
        try (var conn = PgConnection.connect("localhost", 5432, "postgres", "postgres", "postgres")) {
            ...
            conn.enqueue("BEGIN");
            int i1 = conn.enqueue("INSERT INTO orders (product, qty) VALUES ('Gadget', 5) RETURNING id, product");
            int i2 = conn.enqueue("INSERT INTO orders (product, qty) VALUES ('Doohickey', 3) RETURNING id, product");
            conn.enqueue("COMMIT");
            ...
            conn.enqueue("BEGIN");
            for (int i = 0; i < 100; i++) {
                conn.enqueue("INSERT INTO orders (product, qty) VALUES ($1, $2)",
                    "Product-" + i, i + 1);
            }
            conn.enqueue("COMMIT");
            conn.flush();
            ...
            System.out.println("Statement 2 error: " + errorResults.get(1).hasError());
