 ...
 * }</pre>
 *
 * <p><b>Postgres array parameters:</b> Instead of IN-list expansion (which is incompatible
 * with prepared statements because the SQL changes with collection size), Postgres supports passing
 * a Collection or array as a single parameter and using {@code = ANY(:param)}:
 * <pre>{@code
...
public interface PreparedStatement extends AutoCloseable {
   ...
   *
   * <p><b>Postgres:</b> Collection and array values are automatically converted to
   * Postgres array literals for use with {@code = ANY(:param::type[])} syntax.
   *
   * <p><b>MySQL:</b> Collection/array values are not supported — use
   * {@link Connection#query(String, Map)} for queries with IN-list expansion.
   *
  ...
  RowSet query(Map<String, Object> params);
