...
public final class Row {
  ...
  private <T> T decode(
      int index, BiFunction<BinaryCodec, byte[], T> binaryDecode, RawTextDecoder<T> textDecode) {
  ...
  public @Nullable Instant getInstant(int index) {
    ...
    if (isBinary()) {
      return requireBinaryCodec()
          .decodeTimestamptz(copySlice(buf, getOffset(index), getLength(index)))
          .toInstant();
    }
    String s =
