...
class NestedDecoderTest {
    ...
    // =========================================================================
    // DOES NOT WORK: Set<String> from array column
    // =========================================================================

    @Test
    fun `set of strings from JSON array - works via JSON deserialization`() {
        // Sets go through StructureKind.LIST path too, so JSON format works
        val r = row("1", """["foo","bar","baz"]""")
    ...
    @Test
    fun `set of strings from PG array format - fails because parsePgArray returns List`() {
        // parsePgArray returns List<String>, which can't be cast to Set<String>
        val r = row("1", "{foo,bar,baz}")
        assertThrows<ClassCastException> {
            serializer<WithStringSet>().deserialize(RowDecoder(r))
        }
    }

    // =========================================================================
    // DOES NOT WORK: List<Int> from PG array format
    // =========================================================================

    @Test
    fun `list of ints from JSON array - works`() {
        val r = row("1", "[1,2,3]")
    ...
    @Test
    fun `list of ints from PG array format - silently returns strings due to type erasure`() {
        // parsePgArray returns List<String>, cast to List<Int> succeeds due to type erasure,
        // but elements are actually Strings — ClassCastException when accessing as Int
        val r = row("1", "{1,2,3}")
        val result = serializer<WithIntList>().deserialize(RowDecoder(r))
        // This "works" but the list actually contains Strings, not Ints!
        assertEquals(3, result.numbers.size)
        // Prove they are actually Strings masquerading as Ints:
        assertThrows<ClassCastException> {
            @Suppress("USELESS_CAST")
            val first: Int = result.numbers[0] as Int  // actually a String
            first + 1  // force unboxing
        }
    }
}
