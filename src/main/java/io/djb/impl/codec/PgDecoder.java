...
public final class PgDecoder {
    ...
    public BackendMessage readMessage() throws IOException {
        ...
        PgBuffer buf = PgBuffer.wrap(payload);
    ...
    private BackendMessage decode(byte type, PgBuffer buf) {
        return switch (type) {
    ...
    private BackendMessage decodeAuthentication(PgBuffer buf) {
    ...
    private BackendMessage decodeParameterStatus(PgBuffer buf) {
        return new BackendMessage.ParameterStatus(buf.readCString(), buf.readCString());
    }

    private BackendMessage decodeBackendKeyData(PgBuffer buf) {
        return new BackendMessage.BackendKeyData(buf.readInt(), buf.readInt());
    }

    private BackendMessage decodeReadyForQuery(PgBuffer buf) {
        return new BackendMessage.ReadyForQuery((char) buf.readByte());
    }

    private BackendMessage decodeRowDescription(PgBuffer buf) {
    ...
    private BackendMessage decodeDataRow(PgBuffer buf) {
        int columnCount = buf.readUnsignedShort();
    ...
    private BackendMessage decodeCommandComplete(PgBuffer buf) {
    ...
    private BackendMessage decodeErrorResponse(PgBuffer buf) {
    ...
    private BackendMessage decodeNoticeResponse(PgBuffer buf) {
        String severity = null, code = null, message = null, detail = null, hint = null;
        byte fieldType;
        while ((fieldType = buf.readByte()) != 0) {
            String value = buf.readCString();
    ...
    private BackendMessage decodeParameterDescription(PgBuffer buf) {
    ...
    private BackendMessage decodeNotificationResponse(PgBuffer buf) {
        int pid = buf.readInt();
