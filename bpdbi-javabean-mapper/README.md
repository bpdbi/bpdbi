...
A reflection-based `RowMapper` for [bpdbi](../) that automatically maps query result rows to JavaBeans
...
With `bpdbi-kotlin`, use `@Serializable` data classes instead of JavaBeans:
...
- **Java records** → use `bpdbi-record-mapper` (simpler, immutable, reflection-based)
- **Kotlin data classes** → use `bpdbi-kotlin` (compile-time, no reflection)
