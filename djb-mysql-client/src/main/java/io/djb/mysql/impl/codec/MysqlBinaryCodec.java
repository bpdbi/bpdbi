...
/**
 * MySQL binary format decoder. All values are little-endian.
 * Ported from Vert.x MySQL DataTypeCodec.
 */
public final class MysqlBinaryCodec implements BinaryCodec {
    ...
    public LocalTime decodeTime(byte[] value) {
        ...
        if (value.length == 0) return LocalTime.MIDNIGHT;
        int len = value[0] & 0xFF;
        if (len == 0) return LocalTime.MIDNIGHT;
    ...
    public LocalDateTime decodeTimestamp(byte[] value) {
        ...
        int month = value[3] & 0xFF;
        int day = value[4] & 0xFF;
        if (len == 4) return LocalDateTime.of(year, month, day, 0, 0, 0);
        int hour = value[5] & 0xFF;
        int minute = value[6] & 0xFF;
        int second = value[7] & 0xFF;
        if (len == 7) return LocalDateTime.of(year, month, day, hour, minute, second, 0);
        if (len == 11) {
    ...
    public Duration decodeDuration(byte[] value) {
        if (value.length == 0) return Duration.ZERO;
        int len = value[0] & 0xFF;
        if (len == 0) return Duration.ZERO;
