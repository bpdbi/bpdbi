...
/**
 * Encrypts a password with the server's RSA public key for caching_sha2_password full authentication.
 * The password is XOR'd with the nonce before encryption (MySQL protocol requirement).
 *
 * Ported from Vert.x SQL Client (Eclipse Public License 2.0 / Apache License 2.0).
 */
public final class RsaPublicKeyEncryptor {
     ...
     */
    public static byte[] encrypt(byte[] password, byte[] nonce, String serverRsaPublicKey) {
        try {
    ...
    private static RSAPublicKey parseRsaPublicKey(String serverRsaPublicKey) throws Exception {
        String content = serverRsaPublicKey
            .replace("-----BEGIN PUBLIC KEY-----", "")
            .replace("-----END PUBLIC KEY-----", "")
            .replaceAll("\\n", "");
        byte[] keyBytes = Base64.getDecoder().decode(content.getBytes());
