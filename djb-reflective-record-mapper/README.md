# djb-reflective-record-mapper

A reflection-based `RowMapper` for [djb](../) that automatically maps query result rows to Java records.
...
User user = conn.query("SELECT id, name, email FROM users WHERE id = 1")
    .mapFirst(ReflectiveRecordMapper.of(User.class));
```
